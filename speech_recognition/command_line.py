...
class ModelChangeConfig:
    ...
    conformer: ConformerChangeConfig = field(
        default_factory=ConformerChangeConfig)
...
class TranscriptionConfig:
    ...
    # Path to a directory which contains audio files
    ...
    # Used to override the default audio key in dataset_manifest
    audio_key: str = 'audio_filepath'
    # Path to a yaml file of config of evaluation
    eval_config_yaml: Optional[str] = None
    # Significant inference speedup on short-form data due to padding reduction
    ...
    batch_size: int = 32
    num_workers: int = 0
    # Sets mode of work, if True it will add new field transcriptions.
    append_pred: bool = False
    # If you need to use another model name, rather than standard one.
    pred_name_postfix: Optional[str] = None
    # seed number going to be used in seed_everything()
    ...
    # allow to select MPS device (Apple Silicon M-series GPU)
    ...
    multitask_decoding: MultiTaskDecodingConfig = field(
        default_factory=MultiTaskDecodingConfig)
...
def main(cfg: TranscriptionConfig) -> Union[TranscriptionConfig, List[Hypothesis]]:
    ...
    if cfg.compute_dtype != "float32" and cfg.amp:
        raise ValueError(
            "amp=true is mutually exclusive with a compute_dtype other than float32")
    ...
    with open_dict(cfg):
        if isinstance(asr_model, EncDecCTCModel) or (
            isinstance(
                asr_model, EncDecHybridRNNTCTCModel) and cfg.decoder_type == "ctc"
        ):
    ...
    with torch.amp.autocast('cuda' if torch.cuda.is_available() else 'cpu', dtype=amp_dtype, enabled=cfg.amp):
        with torch.no_grad():
            ...
            if hasattr(override_cfg, "prompt"):
                override_cfg.prompt = parse_multitask_prompt(
                    OmegaConf.to_container(cfg.prompt))
    ...
    if cfg.dataset_manifest is not None:
        logging.info(
            f"Finished transcribing from manifest file: {cfg.dataset_manifest}")
        if cfg.presort_manifest:
            transcriptions = restore_transcription_order(
                cfg.dataset_manifest, transcriptions)
    else:
